...
public function payNow($t_id, $id, $name, $add, $contact, $amount)
{
    $stmt = $this->connect()->prepare("INSERT INTO transaction 
        (t_id, id, name, address, contact, amount, status , date) 
         VALUES (?, ?, ?, ?, ?, ?, 'pending' , NOW())");

    $stmt->bind_param("iisssi", $t_id, $id, $name, $add, $contact, $amount);
    ...
    if ($result) {
        ...
        error_log("payNow error: " . $stmt->error);
        return 2;
    }
}
}
